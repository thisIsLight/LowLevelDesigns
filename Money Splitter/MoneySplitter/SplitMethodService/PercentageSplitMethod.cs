...
{
    public class PercentageSplitMethod : ISplitMethod
    {
        public void Split(User payer, List<User> owers, double amount, List<double> shares = null)
        {
            ...
            {
                throw new ArgumentNullException("Shares not Provided for Percentage Split");
            }
            else
            {
                shares = shares.Select(x => (x * amount) / 100).ToList();
                var indexOfShare = 0;
